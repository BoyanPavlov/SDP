...
template <class T>
LinkedListNode<T> *mergeLists2(LinkedListNode<T> *first, LinkedListNode<T> *second)
{
    LinkedListNode<T> *result = first;
    LinkedListNode<T> *iter = result;
    first = first->next;    //missed that one

    int counter = 1;

    while (first || second)// first || second
    {
        //to check if first or seconds are pointer
        if (counter % 2 == 0)
        {
            iter->next = first;
            first = first->next;
...
LinkedListNode<T> *reorderLinkedList(LinkedListNode<T> *first, LinkedListNode<T> *second)
{
    ...
    // LinkedListNode<T> *result = mergeLists(first, second);
    LinkedListNode<T> *result = mergeLists2(first, second);
